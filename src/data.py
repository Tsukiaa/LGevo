...
class KnowledgeGraph(object):
    def __init__(self, data_path):
        ...
        with open(os.path.join(data_path, 'ts.dict'), encoding='utf-8') as fi:
            for line in fi:
                id, time = line.strip().split('\t')
                self.time2id[time] = int(id)
                self.id2time[int(id)] = time
        ...
        with open(os.path.join(data_path, "train.txt"), encoding='utf-8') as fi:
            for line in fi:
                h, r, t, time = line.strip().split('\t')
                h, r, t, time = self.entity2id[h], self.relation2id[r], self.entity2id[t], self.time2id[time]
                self.train_facts.append([h, r, t, time])
        ...
        subs = [x[2] for x in self.train_facts]
        rels = [x[1]+self.relation_size for x in self.train_facts]
        objs = [x[0] for x in self.train_facts]
        tss = [x[3] for x in self.train_facts]

        for i in range(len(subs)):
            self.train_facts.append((subs[i], rels[i], objs[i], tss[i]))


        with open(os.path.join(data_path, "valid.txt"), encoding='utf-8') as fi:
            for line in fi:
                h, r, t, time = line.strip().split('\t')
                h, r, t, time = self.entity2id[h], self.relation2id[r], self.entity2id[t], self.time2id[time]
                self.valid_facts.append((h, r, t, time))
        ...
        subs = [x[2] for x in self.valid_facts]
        rels = [x[1] + self.relation_size for x in self.valid_facts]
        objs = [x[0] for x in self.valid_facts]
        tss = [x[3] for x in self.valid_facts]
        for i in range(len(subs)):
            self.valid_facts.append((subs[i], rels[i], objs[i], tss[i]))

        with open(os.path.join(data_path, "test.txt"), encoding='utf-8') as fi:
            for line in fi:
                h, r, t, time = line.strip().split('\t')
                h, r, t, time = self.entity2id[h], self.relation2id[r], self.entity2id[t], self.time2id[time]
                self.test_facts.append((h, r, t, time))
        ...
        subs = [x[2] for x in self.test_facts]
        rels = [x[1] + self.relation_size for x in self.test_facts]
        objs = [x[0] for x in self.test_facts]
        tss = [x[3] for x in self.test_facts]
        for i in range(len(subs)):
            self.test_facts.append((subs[i], rels[i], objs[i], tss[i]))
    ...
    def process_data(self, train_facts):
        subs = [x[0] for x in train_facts]
        rels = [x[1] for x in train_facts]
        objs = [x[2] for x in train_facts]
        tss = [x[3] for x in train_facts]
        train_facts = np.column_stack((subs, rels, objs, tss))
        return train_facts
...
class TrainDataset(Dataset):
    def __init__(self, graph, batch_size):
        ...
        for h, r, t, time in self.graph.train_facts:
            self.r2instances[r].append((h, r, t, time))
...
class ValidDataset(Dataset):
    def __init__(self, graph, batch_size):
        ...
        for h, r, t, time in facts:
            r2instances[r].append((h, r, t, time))
    ...
    def __getitem__(self, idx):
        ...
        for k, (h, r, t, time) in enumerate(data):
            hr_index = self.graph.encode_hr(h, r)
...
class RuleDataset(Dataset):
    def __init__(self, num_relations, input, first):
        self.rules = list()
        self.num_relations = num_relations * 2
        self.ending_idx = num_relations * 2
        self.padding_idx = num_relations * 2 + 1
        ...
        if first is True:
            for rule in rules:
                rule_len = len(rule)
                formatted_rule = [rule[0:-1] + [self.ending_idx], self.padding_idx, rule[-1]]
    ...
    def collate_fn(data):
        ...
        mask = (target != torch.tensor(padding_index, dtype=torch.long).unsqueeze(1)) & (target != torch.tensor(ending_idx, dtype=torch.long).unsqueeze(1))
